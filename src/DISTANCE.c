...
void ULTRASONIC_Init(void) {
	...
	GPIOA->MODER &= ~GPIO_MODER_MODE0;		// PB6 used as alternative function
	GPIOA->MODER |= GPIO_MODER_MODE0_1;
	GPIOA->PUPDR &= ~GPIO_PUPDR_PUPD0;		// PB6  no pull-up, no pull-down
	GPIOA->AFR[0] &= ~GPIO_AFRL_AFSEL0;
	GPIOA->AFR[0]  |= GPIO_AFRL_AFSEL0_0;	// AF2 for PB6
	
	// setup echo (input capture) pin (PA1)
	GPIOA->MODER &= ~GPIO_MODER_MODE1_0;		// configure PA9 to be used as alternative function
	GPIOA->MODER |= GPIO_MODER_MODE1_1;
	GPIOA->AFR[0] &= ~GPIO_AFRL_AFSEL1;			// Alternate Function AF9
	GPIOA->AFR[0] |= GPIO_AFRL_AFSEL1_1;
	GPIOA->PUPDR &= ~GPIO_PUPDR_PUPD1;			// set PA9 to no pull-up, no pull-down
	GPIOA->OTYPER &= ~GPIO_OTYPER_OT1;		// set output type of PA9 to push-pull
	GPIOA->OSPEEDR |= GPIO_OSPEEDR_OSPEED1;	// set PA9 to very high output speed

	// setup trigger timer (TIM2)
	RCC->APB1ENR1 |= RCC_APB1ENR1_TIM5EN;			// enable Timer 1 
	...
	TIM5->BDTR |= (TIM_BDTR_MOE + TIM_BDTR_OSSR + TIM_BDTR_OSSI);		// break and dead time
	...
	TIM2->DIER |= (TIM_DIER_CC1DE + TIM_DIER_CC1IE + TIM_DIER_UIE); // Enable interrupt, DMA, and Update interrupt
